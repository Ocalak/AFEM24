...
function(gamsss){
  ...
  gamt <- bam(Temp_ACT~s(Hoy,bs="cc",k=20),data = gam_df[169:43088,],discrete = TRUE,select = TRUE)




        gam_df$ftemp <- (gam_df$Temp_ACT-gam_df$Hoy*gamt$sp)^2
        
        s.gam <- bam(ftemp~s(Hoy,bs="cc",k=20),data=gam_df[169:43088,],discrete = TRUE,select = TRUE)
